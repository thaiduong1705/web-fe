        ...
        <div className="fixed top-0 left-0 right-0 bottom-0 z-500 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 max-h-full flex justify-center items-center bg-overlay-70">
            <div className="bg-white rounded-lg shadow dark:bg-gray-700 w-[700px]">
                <div className=" relative flex items-start justify-between p-[16px] border-b rounded-t-[4px] dark:border-gray-600">
                    <h3 className="text-[20px] font-semibold text-gray-900 dark:text-white line-clamp-1 w-[calc(100%-10rem)]">
                        Ứng tuyển: {post?.jobTitle}
                    </h3>
                    <button
                        type="button"
                ...
                <div className="p-[24px] space-y-[24px]">
                    ...
                    <div className="border rounded-[4px] grid grid-rows-3 grid-flow-col gap-[16px] p-[4px]">
                        <p>Tên ứng viên: {selectedCandidate?.candidateName}</p>
                        <p>Email: {selectedCandidate?.email}</p>
                        <p>Địa chỉ: {selectedCandidate?.homeAddress}</p>
                        <p>Điện thoại: {selectedCandidate?.phoneNumber}</p>
                    </div>
                </div>
